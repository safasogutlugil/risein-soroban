...
// Get user eklenir mi?
// Multiple payments
// Get past payments and messages
// Regular payments --> make paymenta süre ve birimi gir. o kadar süre sonra tekrar ödesin
// Testler
// Main

#![no_std]
use soroban_sdk::{contract, contractimpl, contracttype, log, symbol_short, vec, Address, Env, Symbol, Vec, Map};
...
impl PaymentContract {
    pub fn initialize(env: Env) {
        ...
        let deneme: Map<Address, PaymentUser>;
    ...
    pub fn get_users(env: &Env) -> Vec<PaymentUser> {
        let x: Map<Address,PaymentUser> = env.storage().instance().get(&symbol_short!("deneme")).unwrap();
